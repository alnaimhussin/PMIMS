...
clearstatcache(); // Clears file status cache
    ...
    if ($data) :
      foreach ($data->getResult() as $post) : 
        ...
        if ($post->citymunDesc == "ZAMBOANGA CITY") {
          $address    = $post->citymunDesc;
        } else {
    ...
    $qrcode_margin_right = 430;
    $qrcode_margin_bottom = 20;

    // Set the margins of the employee picture
    // $signature_margin_right = 60;
    // $signature_margin_bottom = 80;
    ...
    $qrcode_xcoordest = imagesx($virtual_id_front) - $qrcode_sx - $qrcode_margin_right;
    $qrcode_ycoordest = imagesy($virtual_id_front) - $qrcode_sy - $qrcode_margin_bottom;
    
    // $signature_xcoordest = imagesx($virtual_id_front) - $signature_sx - $signature_margin_right;
    // $signature_ycoordest = imagesy($virtual_id_front) - $signature_sy - $signature_margin_bottom;

    //merges two images into one
    imagecopyresampled($virtual_id_front, $qrcode_final, $qrcode_xcoordest, $qrcode_ycoordest, 0, 0, $qrcode_sx, $qrcode_sy, $qrcode_sx, $qrcode_sy);
    // imagecopyresampled($virtual_id_front, $signature_final, $signature_xcoordest, $signature_ycoordest, 0, 0, $signature_sx, $signature_sy, $signature_sx, $signature_sy);
    ...
    // Set new dimensions for the employee photo
    $new_width = 500;  // Adjust as needed
    $new_height = 600; // Adjust as needed

    // Get original image dimensions
    $original_width = imagesx($virtual_employee_picture);
    $original_height = imagesy($virtual_employee_picture);

    // Coordinates of destination point
    $xcoordest = imagesx($virtual_id_front) - $new_width - $margin_right;
    $ycoordest = imagesy($virtual_id_front) - $new_height - $margin_bottom;

    // Resize and merge the image
    imagecopyresampled(
        $virtual_id_front, 
        $virtual_employee_picture, 
        $xcoordest, $ycoordest, 
        0, 0, 
    ...
    $textcolor_white = imagecolorallocate($image, 255, 255, 255); // text color
    ...
    imagettfstroketext($image, $font_size, 0, 28, $fullname_y, $textcolor, $stroke_color, $font_file_3, $fullname, 4);
    imagettftext($image, 18, 0, 28, 730, $textcolor, $font_file_1, strtoupper($position)); // Add Position to image:
    imagettftext($image, 18, 0, 28, 760, $textcolor, $font_file_1, strtoupper($dept)); // Add Department to image:
    ...
    imagettftext($image, 50, 0, 230, $pos1_y, $textcolor_white, $font_file_2, strtoupper($pgbid)); // Add Department to image:
              ...
              <div class="card-body" style="height: auto;">
                <div class="row">
                  <div class="col-sm-12 col-lg-4">
                    <img src="<?php echo $front . '?t=' . time(); ?>" id="front" alt="" width="80%" height="auto"
                      style="border: 3px solid #555; margin-top: 5px">
                  </div>
                  <div class="col-sm-12 col-lg-4">
                    <img src="<?php echo $back . '?t=' . time(); ?>" id="back" alt="" width="80%" height="auto"
                      style="border: 3px solid #555; margin-top: 5px">
                  </div>
                  <div class="col-sm-12 col-lg-4">
                    <img src="<?php echo $big . '?t=' . time(); ?>" id="big" alt="" width="80%" height="auto"
                      style="border: 3px solid #555; margin-top: 5px">
                  </div>
                </div>
              </div>
...
<script>
    ...
    // Function to strip the cache buster before saving
    function getCleanFileName(path) {
        // Remove query string (the ?t= part)
        let cleanPath = path.split('?')[0];
        // Grab the actual filename from the URL
    ...
    btnDownload.addEventListener('click', () => {
        ...
        // Save using the cleaned filename
        saveAs(imagePath1, getCleanFileName(imagePath1));
        saveAs(imagePath2, getCleanFileName(imagePath2));
        saveAs(imagePath3, getCleanFileName(imagePath3));
    });
